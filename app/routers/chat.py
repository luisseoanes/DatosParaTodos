...
from app.schemas.chat import StartSessionRequest, MessageRequest, ChatResponse, DeleteResponse, BusinessUnderstandingRequest, BusinessUnderstandingResponse
...
@router.post("/business-understanding", response_model=BusinessUnderstandingResponse)
async def generate_business_understanding(req: BusinessUnderstandingRequest):
    """
    Genera automáticamente la Comprensión del Negocio (CRISP-DM fase 1)
    a partir de los nombres de columnas y una muestra de datos.
    """
    try:
        result = await gemini_service.generate_business_understanding(
            columns=req.columns,
            sample=req.sample,
            dataset_name=req.dataset_name,
        )
        return BusinessUnderstandingResponse(**result)
    except Exception as e:
        logger.error(f"Business understanding error: {e}")
        err_str = str(e)
        if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
            import re as _re
            retry_match = _re.search(r"retryDelay.*?(\d+)s", err_str)
            retry_s = retry_match.group(1) if retry_match else "30"
            raise HTTPException(
                status_code=429,
