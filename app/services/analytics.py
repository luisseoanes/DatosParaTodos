...
class AnalyticsService:
    ...
    def detect_columns(self, data: list[dict[str, Any]]) -> dict[str, list[str]]:
        ...
        for col in all_cols:
            ...
            if len(num_vals) > len(vals) * 0.6:
                # Verificar si es una categórica disfrazada de número
                # (estrato 1-6, mes 1-12, hora 0-23, día semana, códigos, etc.)
                floats = [self._to_float(v) for v in vals if self._to_float(v) is not None]
                all_integers = all(f % 1 == 0 for f in floats)
                n_unique = len(set(floats))
                # Columnas de conteo/cantidad/tiempo siempre son numéricas aunque sean enteros pequeños
                _count_hints = ('numero', 'num_', 'cantidad', 'cant_', 'casos', 'total', 'count',
                                'victima', 'muerto', 'herido', 'peaton', 'conductor',
                                'valor', 'monto', 'tasa', 'score', 'punt', 'puntaje',
                                'anno', 'ano', 'a_o', 'anio', 'year', 'periodo')
                is_count_col = any(h in col.lower() for h in _count_hints)
                if all_integers and n_unique <= 15 and not is_count_col:
                    categorical.append(col)  # ordinal → tratar como categórica
                else:
                    numeric.append(col)
    ...
    def run_cleaning_pipeline(self, raw_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for col in cols["numeric"]:
            ...
            # Si hay muy pocos valores únicos, probablemente es una variable de conteo (ej. número de muertos) o categórica.
            if unique_vals < 15:
                continue
            ...
            # Si el IQR es 0 (ej. el 75% de los datos es 0), no acotar, sino todos los >0 serían outliers.
            if iqr == 0:
                continue
        ...
        for row in cleaned:
            ...
            for col in cols["categorical"]:
                value = clean_row.get(col)
                if isinstance(value, str):
                    # Convertir a mayúsculas y remover espacios para consolidar categorías
                    clean_row[col] = value.strip().upper()
